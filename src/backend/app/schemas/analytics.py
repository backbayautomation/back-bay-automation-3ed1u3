...
class UsageMetrics(BaseModel):
    ...
    success_rate: float = Field(ge=0, le=100, description="Query success rate percentage")
...
class DocumentMetrics(BaseModel):
    ...
    def validate_counts(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        type_counts = sum(values.get('document_types', {}).values())

        if failed > total:
            raise ValueError("Failed documents cannot exceed total documents")
        
        if type_counts != total:
...
class TimeSeriesData(BaseModel):
    ...
    metric_name: str = Field(..., min_length=1, description="Name of the metric being tracked")
    ...
    def validate_series(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(ts.tzinfo is not None for ts in timestamps):
            raise ValueError("All timestamps must be in UTC")

        # Verify timestamps are ordered
        if any(t1 >= t2 for t1, t2 in zip(timestamps[:-1], timestamps[1:])):
...
class AnalyticsDashboard(BaseModel):
    ...
    def validate_tenant(cls, value: UUID) -> UUID:
        ...
        # Note: Actual database query would be handled in service layer
        # This is a placeholder for schema-level validation
